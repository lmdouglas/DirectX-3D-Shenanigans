...
Texture::Texture(const wchar_t* full_path): Resource(full_path)
{
	...
	if (SUCCEEDED(res))
	{
		res = DirectX::CreateTexture(
			...
			image_data.GetImages(), image_data.GetImageCount(), image_data.GetMetadata(),
			&m_texture);
		...
		HRESULT res = GraphicsEngine::get()->getRenderSystem()->m_d3d_device->CreateShaderResourceView(m_texture, &desc, &m_shader_res_view);
		if (SUCCEEDED(res))
		{
			bool OK = true;
		}
		...
		{
			throw std::exception("Texture: Could not create shader resource view.");
		}
	}
	else
	{
